...
class RiemannZetaApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Riemann Zeta Fonksiyonu Görselleştirici")
        ...
        ttk.Label(self.control_frame, text="ζ(s) hesapla (s = σ + it için)").grid(column=0, row=0, columnspan=4)
        
        ttk.Label(self.control_frame, text="σ değeri:").grid(column=0, row=1)
        ...
        ttk.Label(self.control_frame, text="t değeri:").grid(column=2, row=1)
        ...
        self.calc_button = ttk.Button(self.control_frame, text="Hesapla", command=self.calculate_point)
        ...
        ttk.Label(self.control_frame, text="3B Yüzey Grafiği Aralığı").grid(column=0, row=4, columnspan=4)
        ...
        ttk.Label(self.control_frame, text="Nokta sayısı:").grid(column=0, row=7)
        self.points_var = tk.IntVar(value=30)
        ttk.Entry(self.control_frame, textvariable=self.points_var, width=5).grid(column=1, row=7)
        
        self.plot_button = ttk.Button(self.control_frame, text="3B Grafik Oluştur", command=self.generate_plot)
        ...
        self.ax.text(0.5, 0.5, "Riemann Zeta Fonksiyonu Görselleştirici\n\nDeğerleri girin ve Hesapla veya Grafik Oluştur düğmesine tıklayın", 
    ...
    def calculate_point(self):
        ...
        if z is not None:
            ...
            self.result_var.set("Zeta fonksiyonu değeri hesaplanırken hata oluştu")
