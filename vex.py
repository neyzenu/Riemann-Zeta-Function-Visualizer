...
class RiemannZetaApp:
    def __init__(self, root):
        ...
        ttk.Label(self.control_frame, text="ζ(s) calculate (s = σ + it)").grid(column=0, row=0, columnspan=4)
    ...
    def calculate_point(self):
        ...
        if z is not None:
            ...
            result_text += f"Magnitude: |ζ({sigma}+{t}i)| = {mag:.6f}\n"
            result_text += f"Phase: Arg(ζ({sigma}+{t}i)) = {phase:.6f} radians"
    ...
    def generate_plot(self):
        ...
        ax1.set_ylabel('t (Imaginary part)')
        ax1.set_zlabel('|ζ(s)|')
        ax1.set_title('Magnitude |ζ(s)|')
        
        ax2 = self.fig.add_subplot(122, projection='3d')
        surf2 = ax2.plot_surface(sigma_grid, t_grid, phase, cmap=cm.hsv, 
                           linewidth=0, antialiased=True)
        ax2.set_xlabel('σ (Real part)')
        ax2.set_ylabel('t (Imaginary part)')
        ax2.set_zlabel('Arg(ζ(s))')
        ax2.set_title('Phase Arg(ζ(s))')
